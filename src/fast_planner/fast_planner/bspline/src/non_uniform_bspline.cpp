...
bool NonUniformBspline::checkFeasibility(bool show) {
  ...
  for (int i = 0; i < P.rows() - 1; ++i) {
    ...
    if (fabs(vel(0)) > limit_vel_ + 1e-4 || fabs(vel(1)) > limit_vel_ + 1e-4 ||
      ...
      if (show)
        cout << "[Check]: Infeasible vel " << i << " :" << vel.transpose()
             << endl;
      fea = false;
  ...
  for (int i = 0; i < P.rows() - 2; ++i) {
    ...
    if (fabs(acc(0)) > limit_acc_ + 1e-4 || fabs(acc(1)) > limit_acc_ + 1e-4 ||
      ...
      if (show)
        cout << "[Check]: Infeasible acc " << i << " :" << acc.transpose()
             << endl;
      fea = false;
...
bool NonUniformBspline::reallocateTime(bool show) {
  ...
  for (int i = 0; i < P.rows() - 1; ++i) {
    ...
    if (fabs(vel(0)) > limit_vel_ + 1e-4 || fabs(vel(1)) > limit_vel_ + 1e-4 ||
      ...
      fea = false;
      if (show)
        cout << "[Realloc]: Infeasible vel " << i << " :" << vel.transpose()
             << endl;
  ...
  for (int i = 0; i < P.rows() - 2; ++i) {
    ...
    if (fabs(acc(0)) > limit_acc_ + 1e-4 || fabs(acc(1)) > limit_acc_ + 1e-4 ||
      ...
      fea = false;
      if (show)
        cout << "[Realloc]: Infeasible acc " << i << " :" << acc.transpose()
             << endl;
