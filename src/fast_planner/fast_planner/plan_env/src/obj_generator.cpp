...
int obj_num;
double _xy_size, _h_size, _vel, _yaw_dot, _acc_r1, _acc_r2, _acc_z, _scale1,
    _scale2, _interval;
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < obj_num; ++i) {
    ros::Publisher pose_pub = node.advertise<geometry_msgs::PoseStamped>(
        "/dynamic/pose_" + to_string(i), 10);
    pose_pubs.push_back(pose_pub);
  }

  ros::Timer update_timer =
      node.createTimer(ros::Duration(1 / 30.0), updateCallback);
  cout << "[dynamic]: initialize with " + to_string(obj_num) << " moving obj."
       << endl;
  ...
  for (int i = 0; i < obj_num; ++i) {
    ...
    Eigen::Vector3d scale(rand_scale(eng), 1.5 * rand_scale(eng),
                          rand_scale(eng));
    ...
    model.setLimits(Eigen::Vector3d(_xy_size, _xy_size, _h_size),
                    Eigen::Vector2d(0.0, _vel), Eigen::Vector2d(0, 0));
...
void visualizeObj(int id) {
  ...
  mk.color.a = 0.5, mk.color.r = color(0), mk.color.g = color(1),
  mk.color.b = color(2);
  ...
  mk.pose.position.x = pos(0), mk.pose.position.y = pos(1),
  ...
  pose.header.seq = id;
  pose.pose.position.x = pos(0), pose.pose.position.y = pos(1),
