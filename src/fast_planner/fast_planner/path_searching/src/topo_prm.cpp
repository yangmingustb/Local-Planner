...
void TopologyPRM::init(ros::NodeHandle& nh) {
  ...
  offset_ = Eigen::Vector3d(0.5, 0.5, 0.5) -
            edt_environment_->sdf_map_->getOrigin() / resolution_;
...
void TopologyPRM::findTopoPaths(Eigen::Vector3d start, Eigen::Vector3d end,
  ...
  std::cout << "\n[Topo]: total time: " << total_time
            << ", graph: " << graph_time << ", search: " << search_time
            << ", short: " << short_time << ", prune: " << prune_time
            << ", select: " << select_time << std::endl;
}

list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start,
                                              Eigen::Vector3d end) {
...
vector<GraphNode::Ptr> TopologyPRM::findVisibGuard(Eigen::Vector3d pt) {
  ...
  for (list<GraphNode::Ptr>::iterator iter = graph_.begin();
...
bool TopologyPRM::needConnection(GraphNode::Ptr g1, GraphNode::Ptr g2,
                                 Eigen::Vector3d pt) {
...
bool TopologyPRM::lineVisib(const Eigen::Vector3d& p1,
                            const Eigen::Vector3d& p2, double thresh,
  ...
  if (graph_.size() > 2) {
    for (list<GraphNode::Ptr>::iterator iter1 = graph_.begin();
      ...
      if ((*iter1)->neighbors_.size() <= 1) {
        ...
        for (list<GraphNode::Ptr>::iterator iter2 = graph_.begin();
             iter2 != graph_.end(); ++iter2) {
          for (vector<GraphNode::Ptr>::iterator it_nb =
...
vector<vector<Eigen::Vector3d>> TopologyPRM::selectShortPaths(
  ...
  for (int i = 0; i < short_paths.size(); ++i) {
    short_paths[i].insert(short_paths[i].begin(), start_pts_.begin(),
                          start_pts_.end());
    short_paths[i].insert(short_paths[i].end(), end_pts_.begin(),
                          end_pts_.end());
  }
  for (int i = 0; i < short_paths.size(); ++i) {
...
vector<Eigen::Vector3d> TopologyPRM::discretizePath(
  ...
  for (int i = 0; i < pt_num; ++i) {
    ...
    double lambda =
        (cur_l - len_list[idx]) / (len_list[idx + 1] - len_list[idx]);
    Eigen::Vector3d inter_pt =
        (1 - lambda) * path[idx] + lambda * path[idx + 1];
...
vector<Eigen::Vector3d> TopologyPRM::pathToGuidePts(
    vector<Eigen::Vector3d>& path, int pt_num) {
  return discretizePath(path, pt_num);
}

void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id,
                               int iter_num) {
  ...
  for (int k = 0; k < iter_num; ++k) {
    ...
    for (int i = 1; i < dis_path.size(); ++i) {
      if (lineVisib(short_path.back(), dis_path[i], resolution_, colli_pt,
                    path_id))
        continue;
  ...
  if (parallel_shortcut_) {
    ...
    for (int i = 0; i < raw_paths_.size(); ++i) {
      short_threads.push_back(
          thread(&TopologyPRM::shortcutPath, this, raw_paths_[i], i, 1));
    }
    for (int i = 0; i < raw_paths_.size(); ++i) {
...
vector<Eigen::Vector3d> TopologyPRM::discretizeLine(Eigen::Vector3d p1,
                                                    Eigen::Vector3d p2) {
...
Eigen::Vector3d TopologyPRM::getOrthoPoint(
...
bool TopologyPRM::triangleVisib(Eigen::Vector3d pt, Eigen::Vector3d p1,
                                Eigen::Vector3d p2) {
