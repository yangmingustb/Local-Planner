...
class TopologyPRM {
 private:
  ...
  void shortcutPaths();
  vector<vector<Eigen::Vector3d>> pruneEquivalent(
  ...
  vector<GraphNode::Ptr> findVisibGuard(
      Eigen::Vector3d pt);  // find pairs of visibile guard
  ...
  vector<Eigen::Vector3d> discretizeLine(Eigen::Vector3d p1,
                                         Eigen::Vector3d p2);
  vector<vector<Eigen::Vector3d>> discretizePaths(
 ...
 public:
  ...
  vector<Eigen::Vector3d> pathToGuidePts(vector<Eigen::Vector3d>& path,
                                         int pt_num);
};
