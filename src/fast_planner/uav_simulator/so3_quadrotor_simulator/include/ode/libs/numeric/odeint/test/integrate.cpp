...
struct perform_integrate_const_test {
  void operator()(const value_type t_end, const value_type dt) {
    std::cout << "Testing integrate_const with " << typeid(Stepper).name()
              << std::endl;
    ...
    // std::cout << t_end << " (" << dt << "), " << steps << " , " <<
    // times.size() << " , " << 10.0+dt*steps << "=" << x_end[0] << std::endl;

    BOOST_CHECK_EQUAL(static_cast<int>(times.size()),
                      static_cast<int>(floor(t_end / dt)) + 1);

    for (size_t i = 0; i < times.size(); ++i) {
      // std::cout << i << " , " << times[i] << " , " << static_cast< value_type
      // >(i)*dt << std::endl;
      // check if observer was called at times 0,1,2,...
      BOOST_CHECK_SMALL(times[i] - static_cast<value_type>(i) * dt,
                        (i + 1) * 2E-16);
    }

    // check first, trivial, component
    BOOST_CHECK_SMALL((10.0 + times[times.size() - 1]) - x_end[0],
...
struct perform_integrate_adaptive_test {
  void operator()(const value_type t_end = 10.0, const value_type dt = 0.03) {
    std::cout << "Testing integrate_adaptive with " << typeid(Stepper).name()
              << std::endl;
    ...
    size_t steps = integrate_adaptive(Stepper(), *lorenz, x, 0.0, t_end, dt,
                                      push_back_time(times, x_end));

    //        std::cout << t_end << " , " << steps << " , " << times.size() << "
    ...
    // check first, trivial, component
    BOOST_CHECK_SMALL((10.0 + t_end) - x_end[0],
                      1E-6);  // precision of steppers: 1E-6
                              //        BOOST_CHECK_EQUAL( x[1] , x_end[1] );
                              //        BOOST_CHECK_EQUAL( x[2] , x_end[2] );
  }
...
struct perform_integrate_times_test {
  void operator()(const int n = 10, const int dn = 1,
                  const value_type dt = 0.03) {
    std::cout << "Testing integrate_times with " << typeid(Stepper).name()
              << std::endl;
    ...
    for (int i = 0; boost::numeric::odeint::detail::less_with_sign(
             i, static_cast<int>(obs_times.size()), dt);
         i += dn) {
      obs_times[i] = i;
    }
    // simple stepper
...
struct perform_integrate_n_steps_test {
  void operator()(const int n = 200, const value_type dt = 0.01) {
    ...
    // simple stepper
    value_type end_time = integrate_n_steps(Stepper(), lorenz, x, 0.0, dt, n,
                                            push_back_time(times, x_end));
    ...
    // check first, trivial, component
    BOOST_CHECK_SMALL((10.0 + end_time) - x_end[0],
                      1E-6);  // precision of steppers: 1E-6
                              //        BOOST_CHECK_EQUAL( x[1] , x_end[1] );
                              //        BOOST_CHECK_EQUAL( x[2] , x_end[2] );
  }
...
class stepper_methods
    : public mpl::vector<
          euler<state_type>, modified_midpoint<state_type>,
          runge_kutta4<state_type>, runge_kutta_cash_karp54<state_type>,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(integrate_const_test_case, Stepper,
                              stepper_methods) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(integrate_adaptive_test_case, Stepper,
                              stepper_methods) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(integrate_times_test_case, Stepper,
                              stepper_methods) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(integrate_n_steps_test_case, Stepper,
                              stepper_methods) {
