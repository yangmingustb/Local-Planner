...
struct push_back_time {
  ...
  void operator()(const state_type &x, double t) { m_times.push_back(t); }
};
...
struct perform_integrate_adaptive_test {
  void operator()(void) {
    ...
    size_t steps =
        integrate_adaptive(Stepper(), std::make_pair(sys(), jacobi()), x, 0.0,
                           t_end, dt, push_back_time(times));
...
struct perform_integrate_times_test {
  void operator()(void) {
    ...
    integrate_times(Stepper(), std::make_pair(sys(), jacobi()), x,
                    ...
                    boost::counting_iterator<int>(10), dt,
...
struct perform_integrate_n_steps_test {
  void operator()(void) {
    ...
    // simple stepper
    value_type end_time =
        integrate_n_steps(Stepper(), std::make_pair(sys(), jacobi()), x, 0.0,
                          dt, n, push_back_time(times));
...
class stepper_methods
    : public mpl::vector<rosenbrock4<value_type>,
                         rosenbrock4_controller<rosenbrock4<value_type> >,
                         rosenbrock4_dense_output<rosenbrock4_controller<
