...
struct hamiltonian {
  ...
  void initialize_kinetic_term() {
    ...
    mtl::matrix::inserter<matrix_type, mtl::update_plus<complex<double> > > ins(
        m_H);
    const double z = 1.0;
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < N; ++i) {
    x[i] = exp(-(i - N0) * (i - N0) / (4.0 * sigma0 * sigma0)) *
           exp(complex<double>(0.0, k0 * i));
  ...
  integrate_const(stepper(), hamiltonian(V), x, 0.0, 1000.0, 0.1,
                  write_for_gnuplot(10));
