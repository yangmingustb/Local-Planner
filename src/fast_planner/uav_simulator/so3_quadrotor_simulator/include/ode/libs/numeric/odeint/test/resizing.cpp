...
void constant_system(const test_array_type &x, test_array_type &dxdt,
                     double t) {
  dxdt[0] = 1.0;
}
...
typedef euler<test_array_type, double, test_array_type, double, range_algebra,
...
typedef runge_kutta4_classic<test_array_type, double, test_array_type, double,
                             range_algebra, default_operations, never_resizer>
    rk4_manual_type;
typedef runge_kutta4_classic<test_array_type, double, test_array_type, double,
...
typedef runge_kutta4<test_array_type, double, test_array_type, double,
                     range_algebra, default_operations, never_resizer>
    rk4_gen_manual_type;
typedef runge_kutta4<test_array_type, double, test_array_type, double,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_resize, T, resize_check_types) {
  ...
  BOOST_CHECK_MESSAGE(
      adjust_size_count == resize_calls * multiplicity,
      "adjust_size_count : " << adjust_size_count
                             << " expected: " << resize_calls * multiplicity);
}
