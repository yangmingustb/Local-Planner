...
class rk4_decorator {
 public:
  ...
  void do_step(System system, const StateIn &in, const DerivIn &dxdt,
               value_type t, StateOut &out, value_type dt) {
    m_stepper.do_step(system, in, dxdt, t, out, dt);
    ++do_count;
  }

  template <class System, class StateInOut, class DerivIn>
  void do_step(System system, StateInOut &x, const DerivIn &dxdt, value_type t,
               value_type dt) {
...
BOOST_AUTO_TEST_CASE(test_copying) {
  ...
  stepper_type s2(s1);
  BOOST_CHECK_CLOSE(s1.step_storage()[0].m_v[0], s2.step_storage()[0].m_v[0],
                    1.0e-14);
  BOOST_CHECK_CLOSE(s1.step_storage()[1].m_v[0], s2.step_storage()[1].m_v[0],
                    1.0e-14);
  BOOST_CHECK((&(s1.step_storage()[0])) != (&(s2.step_storage()[0])));

  stepper_type s3;
  state_type *p1 = &(s3.step_storage()[0].m_v),
             *p2 = &(s3.step_storage()[1].m_v);
  s3 = s1;
  BOOST_CHECK(p1 == (&(s3.step_storage()[0].m_v)));
  BOOST_CHECK(p2 == (&(s3.step_storage()[1].m_v)));

  BOOST_CHECK_CLOSE(s1.step_storage()[0].m_v[0], s3.step_storage()[0].m_v[0],
                    1.0e-14);
  BOOST_CHECK_CLOSE(s1.step_storage()[1].m_v[0], s3.step_storage()[1].m_v[0],
                    1.0e-14);
}
...
BOOST_AUTO_TEST_CASE(test_auto_initialization) {
  ...
  adams_bashforth<3, state_type, value_type, state_type, value_type,
                  range_algebra, default_operations, initially_resizer,
