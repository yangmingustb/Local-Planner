...
struct fpu {
  ...
  double energy(const container_type &q, const container_type &p) const {
    ...
    for (size_t i = 0; i < n - 1; ++i) {
      tmp = q[i + 1] - q[i];
      energy += 0.5 * (p[i] * p[i] + tmp * tmp) +
                0.25 * m_beta * tmp * tmp * tmp * tmp;
    }
  ...
  void local_energy(const container_type &q, const container_type &p,
                    container_type &e) const {
    // ...
    //<-
    size_t n = q.size();
    double tmp = q[0];
...
int main(int argc, char **argv) {
  ...
  integrate_const(stepper_type(), fpu_instance,
                  make_pair(boost::ref(q), boost::ref(p)), 0.0, 1000.0, dt,
