...
struct heun_a1 : boost::array<Value, 1> {
  heun_a1(void) { (*this)[0] = static_cast<Value>(1) / static_cast<Value>(3); }
};
...
template <class State, class Value = double, class Deriv = State,
          class Time = Value,
...
class heun
    : public boost::numeric::odeint::explicit_generic_rk<
          3, 3, State, Value, Deriv, Time, Algebra, Operations, Resizer> {
 public:
  typedef boost::numeric::odeint::explicit_generic_rk<
      3, 3, State, Value, Deriv, Time, Algebra, Operations, Resizer>
      stepper_base_type;
