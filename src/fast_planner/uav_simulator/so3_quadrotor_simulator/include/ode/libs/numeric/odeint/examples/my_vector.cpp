...
class my_vector {
 ...
 public:
  my_vector(const size_t N) : m_v(N) { m_v.reserve(MAX_N); }

  my_vector() : m_v() { m_v.reserve(MAX_N); }

  // ... [ implement container interface ]
  //]
  const double &operator[](const size_t n) const { return m_v[n]; }

  double &operator[](const size_t n) { return m_v[n]; }
