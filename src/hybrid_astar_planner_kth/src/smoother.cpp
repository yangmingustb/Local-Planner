...
//   float edgDst; //todo
//   // the vector determining where the obstacle is
//   Vector2D obsVct(xi.getX() -
//   voronoi.data[(int)xi.getX()][(int)xi.getY()].obstX,
//                     xi.getY() -
//                     voronoi.data[(int)xi.getX()][(int)xi.getY()].obstY);
...
//       float PvorPtn_PedgDst = alpha * obsDst * std::pow(obsDst - vorObsDMax,
//       2) / (std::pow(vorObsDMax, 2)
//                               * (obsDst + alpha) * std::pow(edgDst + obsDst,
//                               2));

//       float PvorPtn_PobsDst = (alpha * edgDst * (obsDst - vorObsDMax) *
//       ((edgDst + 2 * vorObsDMax + alpha)
//                                * obsDst + (vorObsDMax + 2 * alpha) * edgDst +
//                                alpha * vorObsDMax))
//                               / (std::pow(vorObsDMax, 2) * std::pow(obsDst +
//                               alpha, 2) * std::pow(obsDst + edgDst, 2));
//       gradient = wVoronoi * PvorPtn_PobsDst * PobsDst_Pxi + PvorPtn_PedgDst *
//       PedgDst_Pxi;
