...
////                                         COST TO GO
////###################################################
// void Node3D::updateH(const Node3D& goal, const
// nav_msgs::OccupancyGrid::ConstPtr& grid, Node2D* nodes2D, float*
...
////    if (Constants::dubinsLookup && uX < Constants::dubinsWidth - 1 && uY <
///Constants::dubinsWidth - 1) {
...
////        h1 = (int)(helper::normalizeHeadingRad(M_PI_2 - goal.t) /
///Constants::deltaHeadingRad);
////      }
////      // mirror on y axis
////      else if (X <= 0 && Y >= 0) {
...
///Constants::deltaHeadingRad);
////        h1 = (int)(helper::normalizeHeadingRad(M_PI - goal.t) /
///Constants::deltaHeadingRad);
...
//        /*if (Constants::dubinsShot && std::abs(x - goal.x) >= 10 &&
//        std::abs(y - goal.y) >= 10)*/
////      // start
////      double q0[] = { x, y, t};
////      // goal
...
//  if (Constants::twoD && !nodes2D[(int)y * grid->info.width +
//  (int)x].isDiscovered()) {
...
//    nodes2D[(int)y * grid->info.width + (int)x].setG(Algorithm::aStar(goal2d,
//    start2d, grid, nodes2D, visualization));
//  }

//  if (Constants::twoD) {
//    // offset for same node in cell
//    twoDoffset = sqrt(((x - (long)x) - (goal.x - (long)goal.x)) * ((x -
//    (long)x) - (goal.x - (long)goal.x)) +
//                      ((y - (long)y) - (goal.y - (long)goal.y)) * ((y -
//                      (long)y) - (goal.y - (long)goal.y)));
//    twoDCost = nodes2D[(int)y * grid->info.width + (int)x].getG() -
//    twoDoffset;
...
////                                 COLLISION CHECKING
////###################################################
// bool Node3D::isTraversable(const nav_msgs::OccupancyGrid::ConstPtr& grid,
// Constants::config* collisionLookup) const {
//  int X = (int)x;
//  int Y = (int)y;
...
//  int idx = iY * Constants::positionResolution * Constants::headings + iX *
//  Constants::headings + iT;
//  int cX;
//  int cY;
...
//    if (cX >= 0 && (unsigned int)cX < grid->info.width && cY >= 0 && (unsigned
//    int)cY < grid->info.height) {
...
////                                        DUBINS SHOT
////###################################################
// Node3D* Node3D::dubinsShot(const Node3D& goal, const
// nav_msgs::OccupancyGrid::ConstPtr& grid, Constants::config* collisionLookup)
// const {
//  // start
//  double q0[] = { x, y, t };
//  // goal
